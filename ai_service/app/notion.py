...
from .models import AIDocument, AIContext, NotionProgressEntry, PlannerSnapshot
...
class NotionClient:
    ...
    def retrieve_database(self, database_id: str) -> dict[str, Any]:
        normalized = _normalize_database_id(database_id)
        return self._request("GET", f"/databases/{normalized}")

    def retrieve_data_source(self, data_source_id: str) -> dict[str, Any]:
        normalized = _normalize_database_id(data_source_id)
        return self._request("GET", f"/data_sources/{normalized}")
    ...
    def query_database(
        ...
        *,
        ...
        return self._request("POST", f"/databases/{normalized}/query", json=payload)
    ...
    def query_data_source(
        ...
        *,
        ...
        if start_cursor:
            payload["start_cursor"] = start_cursor
        return self._request("POST", f"/data_sources/{normalized}/query", json=payload)
    ...
    def create_page(self, *, parent_database_id: str, properties: dict[str, Any], children: list[dict[str, Any]]) -> dict[str, Any]:
        normalized = _normalize_database_id(parent_database_id)
        payload = {
            "parent": {"data_source_id": normalized},
...
def validate_notes_database(client: NotionClient, database_id: str) -> None:
    resolve_data_source(client, database_id)
...
def get_database_schema(client: NotionClient, database_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "properties": [
            {
                "id": property_payload.get("id", name),
                "name": name,
                "type": property_payload.get("type", "unknown"),
            }
...
def _extract_rich_text(properties: dict[str, Any], allowed_names: set[str] | None = None) -> str:
    ...
    for name, value in properties.items():
        ...
        if value_type == "rich_text":
            ...
            name = value.get("select", {}).get("name")
            if name:
...
def _score_candidate(
    ...
    *,
    ...
    if context_title := next((term for term in search.priority_terms if " " not in term and term in title_text), None):
    ...
    updated_raw = candidate.last_edited_time
    if updated_raw:
        try:
    ...
    *,
    ...
    for block in payload.get("results", []):
        ...
        if block.get("has_children") and depth < max_depth and remaining > 0:
            child_texts = _retrieve_blocks_recursive(
                client,
                block["id"],
                depth=depth + 1,
                max_depth=max_depth,
...
def _build_document_content(candidate: NotionCandidate, reasons: list[str]) -> tuple[str, str]:
    content_sections = [
        ...
        f"Why relevant: {', '.join(reasons[:3]) or 'matched current planner context'}",
    ]
    ...
    *,
    ...
    for candidate in shortlisted:
        block_text = _normalize_text(
            "\n".join(_retrieve_blocks_recursive(client, candidate.page_id, limit=NOTION_MAX_BLOCKS))
        )[:NOTION_MAX_BLOCK_TEXT]
